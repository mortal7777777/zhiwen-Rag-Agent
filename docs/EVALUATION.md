...
- **检索优化后基线（2026-08-26，同题库同裁判口径；l2-kb-005 已正例化、
  l2-kb-004 参考修正为库内表述，索引已按 child 320/64 + 块级去重重建）**：

| id | faithfulness | answer_correctness | context_precision | context_recall |
|---|---|---|---|---|
| l1-kb-001 实事求是 | 0.800 | 0.250 | 0.917 | 0.000 |
| l1-kb-002 示例书 | 0.909 | 0.222 | 0.500 | 1.000 |
| l1-kb-007 示例书 | 1.000 | 0.000 | 0.750 | 0.000 |
| l2-kb-003 主次矛盾 | 1.000 | 0.364 | 1.000 | 1.000 |
| l2-kb-004 示例书 | 1.000 | 0.667 | 1.000 | 1.000 |
| l2-kb-005 造物奥秘(正例) | 1.000 | 0.261 | 0.583 | 0.667 |
| l2-kb-006 综合多跳 | 1.000 | 0.000 | 0.000 | 0.000 |
| l2-kb-008 普遍特殊性 | 1.000 | 0.370 | 1.000 | 1.000 |
| **均值** | **0.958** | **0.267** | **0.719** | **0.583** |

  vs 08-25 基线：**context_recall +0.271（0.312→0.583，验收 +0.2 达标）、
  context_precision +0.243（0.476→0.719，验收不跌达标）、faithfulness +0.047**。
  改动：query_expander 书名号强制扩展 + 关键词扩充、child 320/64、
  RECALL_K 60 / CANDIDATE_POOL 32、块级去重（63,979→44,955 块）、
  索引全量重建。
  仍为 0 的三题需分别看待：
  - l1-kb-001/007：答案正确且引用原文，但库内无 reference 的标准措辞
    （"一切从实际出发/实践检验真理"、"觉察/内在平静"），judge 严格逐句比对
    打 0——属 reference 与库内表述错位，非检索失败（faithfulness 均 ≥0.8）；
  - l2-kb-006（综合多跳）：真检索缺陷——多跳查询命中偏到《示例书》写作背景，
    核心段落未进 top4，是后续 P1 优化（Contextual 前缀/多查询路由）的主目标。
