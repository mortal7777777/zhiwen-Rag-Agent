...
| 症状 | 数据 | 根因 |
|---|---|---|
| 单跳题召回 0 | l1-kb-002/007 书名题 recall 0 | 13-15 字书名题不含"总结/分析"等词,被 `_is_simple` 判定为简单问题**跳过查询扩展** → 原查询与库内表述错位;child 220 字可能切散概述性内容 |
| 多跳题召回 0 | l2-kb-006 综合题 recall 0 | 两篇文档只有一篇的 parent 进候选池 |
| precision 0.476 | 负例 0(正常)+ 部分题 1.0 | reranker 有效,平均被负例拉低;整体可接受 |
| 题目设计 bug | l2-kb-005"负例" | 该负例书**其实在库**(另一本书内含其章节),reference 写"未收录"错误,分数无意义 |
...
- 扩展查询质量正常(4 条, HyDE 含"主要矛盾规定影响其他矛盾"),但 RRF 融合后
  40 候选里 **36 个是同书 epub+pdf 双格式**(同书两格式互相竞争),"主要矛盾"
...
- GraphRAG(长篇幅著作类概念网络有潜力,成本 3-5 倍,不急)
